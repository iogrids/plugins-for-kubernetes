...
function logged_in_redirect() {
	...
	if (file_exists('202-config.php') &&file_exists('202-config-sample.php')) {
	    $re = '/(\$\w*)\s*(=)\s*\'(.*)\';/i';
	    $handle = fopen('202-config.php', 'r');
	    $handle2 = fopen('202-config-sample.php', 'r');
	    $old = '';
	    $new = '';
	    while(!feof($handle)) {
	        $line = fgets($handle);
	        ...
	        $pattern = '/(\w+) (\d+), (\d+)/i';
	        $replacement = "$1 = '';";
	        $line=     preg_replace($re, $replacement, $line);
...
if ((AUTH::logged_in() || AUTH::remember_me_on_logged_out()) AND !isset($_GET['customers_api_key'])) {
	logged_in_redirect();
}
...
if ( $_SERVER['REQUEST_METHOD'] == 'POST' or isset($_REQUEST['sig']) ) {
	...
	if($mysql['user_api']!=''){
	        ...
	        if($api_validate['msg']=="Key valid" || $api_validate['msg']=="Key free"){
	            //update the api key
	            ...
	            $error['api'] = 'Your API Key is incorrect or invalid. <a href="https://my.tracking202.com/api/customers/login?redirect=get-api">Click Here To Get Your API Key</a>';
	        }
	}
	...
	//RECORD THIS USER LOGIN, into user_logs
		...
		$mysql['ip_address'] = $db->real_escape_string ( $_SERVER['HTTP_X_FORWARDED_FOR'] ); 
		...
		if ($error) { 
			...
			if ($slack) 
				$slack->push('failed_login', array('username' => $_POST['user_name'], 'ip' => $_SERVER['HTTP_X_FORWARDED_FOR']));
		} else {
	...
	if (!$error) {
		...
		$ip_id = INDEXES::get_ip_id($ip_address);
		...
		$_SESSION['install_hash'] = $user_row['install_hash'];
		$_SESSION['user_api_key'] = $user_row['p202_customer_api_key'];
		$_SESSION['user_api_key_type'] = $api_validate['msg'];
		...
		$_SESSION['mysql_version'] = getMYSQLVersion($db);
...
if($_GET['customers_api_key']){
	$validate= json_decode(api_key_validate($_GET['customers_api_key']), true);
}else{
	$validate['msg']='';
}
...
echo "<br><center><p>Your Prosper202 ClickServer API Key Is Missing</p></center><br>
       ...
       ";
 }else if($validate['msg'] != 'Key valid' && isset($_GET['customers_api_key'])){
	echo "<br><center><p>Your Prosper202 ClickServer API Key Is Invalid</p></center><br>
		   <a class='btn btn-lg btn-p202 btn-block' type='submit' href='https://my.tracking202.com/api/customers/login?redirect=get-api'>Click Here To Get & Save Your API Key into Prosper202 ClickServer</a><br><br><br><br><br><br>
		   <img src='https://my.tracking202.com/api/v2/dni/deeplink/cookie/set/".base64_encode($strProtocol .  $_SERVER['SERVER_NAME'] . get_absolute_url())."'>	
		   ";
	 }
      ...
      <form class="form-signin form-horizontal" role="form" method="post" action="" id="login-form">
	      ...
	      <?php } ?>
				...
				if((!$user_row['p202_customer_api_key'] && $_GET['customers_api_key'] != '') ||  $validate['msg'] == 'Key valid'){?>
...
<div id='div-gpt-ad-1398648278789-0' style='width:288px; height:300px;'><script type='text/javascript'>googletag.cmd.push(function() { googletag.display('div-gpt-ad-1398648278789-0'); });</script></div>
<?php }?>
</div>
    </div>
    </div>
