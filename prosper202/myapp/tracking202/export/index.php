...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	$error = false;
	if (empty($_POST['conversion_name']) || empty($_POST['time_from']) || empty($_POST['time_to'])) {
		$error = true;
	} else {
		...
		//fputcsv($output, array('Parameters:TimeZone='.$timezoneOffset.",,,,"));
		
		$line=array('Parameters:TimeZone='.$timezoneOffset.';,,,,');
		fwrite($output, implode(',', $line) . "\r\n");

		$line=array('Google Click ID','Conversion Name','Conversion Time','Conversion Value','Conversion Currency');
		fwrite($output, implode(',', $line) . "\r\n");
	
	    if($_POST['aff_campaign']=='--'){
	       $_POST['aff_campaign']=0;
	       $op= '!=';
	    }else{
	        $op= '=';
	    }
		...
		$gclid_sql = "SELECT aff_campaign_name, 202_conversion_logs.click_payout, conv_time, gclid, aff_campaign_currency FROM 202_clicks
					  JOIN 202_conversion_logs USING (click_id)
					  JOIN 202_aff_campaigns USING (aff_campaign_id)
				      JOIN 202_google USING (click_id)
					  WHERE 202_clicks.user_id = 1 
					  AND aff_campaign_id $op '".$mysql['aff_campaign']."'
					  AND gclid != ''
					  AND click_lead = 1 
					  AND 202_clicks.click_time >= '".$time_from."' 
					  AND 202_clicks.click_time <= '".$time_to."'";

		$gclid_results = $db->query($gclid_sql);

		while ($gclid_row = $gclid_results->fetch_assoc()) {
			$campaign_name = $gclid_row['aff_campaign_name'];
			fwrite($output, implode(',', array($gclid_row['gclid'], $_POST['conversion_name'], date('m/d/Y g:i:s A', $gclid_row['conv_time']), number_format((float)$gclid_row['click_payout'], 2, '.', ''), $gclid_row['aff_campaign_currency'])). "\r\n");
		}
