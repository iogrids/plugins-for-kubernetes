...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($_POST['display_ads'] == true) {
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://my.tracking202.com/api/v2/premium-p202/upload-ads/".$user_row['install_hash']."/".$user_row['p202_customer_api_key']."");
	...
	if (!$error) { 
		...
		if ($editing == true) {
			...
			header('location: '.get_absolute_url().'tracking202/setup/ads.php');   
...
if ((($editing == true) or ($add_success != true)) and ($mysql['aff_campaign_id'])) {
    //now grab the Category id, per that campaign id
						...
						<div class="tab-pane active" id="tab_default_1">
								...
								<div class="row">
									<div class="col-xs-7">
										...
										<span class="infotext">Here you can add different text ads you might use with your marketing.</span>
												    ...
												    <?php if ($editing == true) { ?>
													    <div class="row">
													    	...
													    	<div class="col-xs-6">
																...
																<button type="submit" class="btn btn-sm btn-danger btn-block" onclick="window.location='<?php echo get_absolute_url();?>tracking202/setup/ads.php'; return false;">Cancel</button>					    		</div>
													    	</div>
												    <?php } else { ?>
									...
									<div class="col-xs-4 col-xs-offset-1">
										<div class="panel panel-default">
											...
											<?php if($_SESSION['user_pref_ad_settings']!='hide_all'){?>
		                                       <iframe id="adframe" src="<?php echo TRACKING202_ADS_URL; ?>/prosper202-textads/?t202aid=<?php echo $_SESSION['user_cirrus_link']; ?>" scrolling="no" frameborder="0"> </iframe>
		                                      <?php }?>	
												<ul>        
...
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
// Set the url
curl_setopt($ch, CURLOPT_URL, 'https://my.tracking202.com/feed/displayads/');
// Execute
$result = curl_exec($ch);
curl_close($ch);
