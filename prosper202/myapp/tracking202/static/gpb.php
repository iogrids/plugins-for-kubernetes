...
if($postjson && $_SERVER['HTTP_X_SHOPIFY_HMAC_SHA256']){
    $postbody=json_decode($postjson,true);
    $lineItems=$postbody['line_items'][0]['properties'];
    $totalPrice=$postbody['total_price'];
    foreach ($lineItems as $lineItem){
        if(strtolower($lineItem['name'])=='t202subid')
            $the_request['subid'] = $lineItem['value'];
    }
    $_GET['amount']=$totalPrice;
}
...
if (is_numeric($mysql['click_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (empty($mysql['txid'])) {
            ...
            if ($row['click_lead'] == 0) {
                addConversionLog($mysql['click_id'], $mysql['txid'], $mysql['campaign_id'], $mysql['click_payout_added'], $mysql['user_id'], $mysql['click_time'], $ip_address, $_SERVER['HTTP_USER_AGENT'], null, '2');
            } else {
                $click_sql = "
                    UPDATE
                        202_conversion_logs
        ...
        $mysql['aff_campaign_id'] = $db->real_escape_string($cvar_sql_row['aff_campaign_id']);
        $mysql['user_account_currency'] = $db->real_escape_string($cvar_sql_row['user_account_currency']);
        $mysql['aff_campaign_currency'] = $db->real_escape_string($cvar_sql_row['aff_campaign_currency']);
        $mysql['aff_campaign_foreign_payout'] = $db->real_escape_string($cvar_sql_row['aff_campaign_foreign_payout']);
