...
if(trackingEnabled()){
	   ...
	   //legacy cookies
	   setcookie('p202_ipx-legacy', $ipx_id, $expire, '/', $domain);

	  //samesite=none secure cookies
	  if (PHP_VERSION_ID < 70300) {
		  header('Set-Cookie: p202_ipx='.$ipx_id.';max-age='.$expire_header.';Path=/;Domain='.$domain.';SameSite=None; Secure');        
	  }
	  else {
		  setcookie('p202_ipx' , $ipx_id, ['expires' => $expire,'path' => '/','domain' => $domain,'secure' => $secure,'httponly' => $httponly,'samesite' => 'None']);
	  }
}    
