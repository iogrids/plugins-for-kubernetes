	...
	foreach($subids as $subid) {
		...
		if(is_numeric($mysql['click_id'])) {
			...
			if ($result->num_rows == 0) {
				...
				addConversionLog($mysql['click_id'], $mysql['txid'], $mysql['aff_campaign_id'], $mysql['click_payout_added'], $mysql['user_id'], $mysql['click_time'], $ip_address, $_SERVER['HTTP_USER_AGENT'], $mysql['conv_time'], 0);
				...
				if($row['deleted']==1){ //set the lead count to 1

				    $mysql['click_lead']=1;
				}
				else{ //keep original lead count because we are just updating data on a lead that's alraedy counted
					
					if($click_row['click_lead']==0){
						$mysql['click_lead']=1;
					}else{
						$mysql['click_lead'] = $click_row['click_lead'];
					}
				}
				...
				if ($mysql['click_cpa']) {
				    $sql_set = "click_cpc='".$mysql['click_cpa']."', click_lead='".$mysql['click_lead']."', click_filtered='0', click_payout='".$mysql['click_payout']."'";
				    $sql = "UPDATE 202_clicks SET click_cpc='".$mysql['click_cpa']."' click_payout='".$mysql['adjusted_payout']."', click_lead='".$mysql['click_lead']."' WHERE click_id = '".$mysql['click_id']."'";
				} else {
				    $sql = "UPDATE 202_clicks SET click_payout='".$mysql['adjusted_payout']."', click_lead='".$mysql['click_lead']."' WHERE click_id = '".$mysql['click_id']."'";
				}
				...
				$click_sql = "
					UPDATE
						202_conversion_logs
					SET
						click_payout = '".$mysql['adjusted_payout']."',
						deleted = '0'    
					WHERE
...
<div class="row" style="margin-bottom: 15px;">
	...
	<div class="col-xs-12">
		<small>Here is where you can update your income for Prosper202, by importing your subids from your marketing reports.</small>
	</div>
</div>
	...
	<div class="col-xs-12">
		<form method="post" action="" class="form-horizontal" role="form">
			<div class="form-group" style="margin:0px 0px 15px 0px;">
				...
				<div>Format: SUBID, PAYOUT (optional), TRANSACTION ID (optional), TIMESTAMP (m/d/y h:m:s PM/AM optional)</div> 			  
			</div>
			<div class="col-xs-3">
