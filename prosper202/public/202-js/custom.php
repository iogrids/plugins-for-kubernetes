...
$(document).ready(function() {
	...
	updatePlaceholder = function (event) {
		if($(event.target).val() == 6 ){ //only run the link assist if it's Bot202 FB Pixel Assistant
		 ...
		 $('label[for=pixel_code]')[0].textContent = 'Facebook Pixel Id:'
...
$(document).on('change', 'select[name=rule_type]', function() {
	...
	switch(val) {
		...
		case 'fraud':
	  		select.tokenfield('destroy');
			select.attr('placeholder', 'Type true, false or Unknown and hit Enter');
			rotator_tags_autocomplete_truefalse(select_id);
	  	break;
...
function rotator_tags_autocomplete_truefalse(selector){
	...
	elt.tokenfield('setTokens', [{value: 'true', label: 'True'}, {value: 'false', label: 'False'}]);
