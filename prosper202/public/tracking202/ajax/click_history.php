...
<div class="row" style="margin-top: 10px;">
	...
	<div class="col-xs-6 text-right" style="top: -10px;">
		<img style="margin-bottom: 2px;"
			src="<?php echo get_absolute_url();?>202-img/icons/16x16/page_white_excel.png" />
		<a style="font-size: 12px;" target="_new"
			href="<?php echo get_absolute_url();?>tracking202/visitors/download/">
...
//if there is no clicks to display let them know :(
	if ($click_result->num_rows == 0) { 
		?><div
					style="text-align: center; font-size: 14px; border-bottom: 1px rgb(234, 234, 234) solid; padding: 10px;">You
					have no data to display with your above filters currently.</div><?php 		if ($_GET['spy'] == 1) { 
			?><div
					style="text-align: center; font-size: 14px; border-bottom: 1px rgb(234, 234, 234) solid; padding: 10px;">The
					spy view only shows clicks activity within the past 24 hours.</div><?php 		}        
	}    
...
//now display all the clicks
	while ($click_row = $click_result->fetch_array(MYSQLI_ASSOC)) {   
		...
		if ($click_row['aff_campaign_name'] != null && $click_row['rotator_id'] != null){
			...
			$html['aff_campaign_name'] = htmlentities("ALP: ".$click_row['landing_page_nickname'], ENT_QUOTES, 'UTF-8');
		}
        ...
        if ($inet6_ntoa == '') {
            if (! ctype_print($data_row)) { // binary value so decode and return
                $click_row['ip_address'] = inet6_ntoa($click_row['ip_address']);
            }
		}
		...
		<?php } ?>
			...
			<tr
					<?php if (($diff <= 5) and ($new == true)) {echo 'class="new-click" style="display:none;"';}?>>
					...
					<td class="geo"><span data-toggle="tooltip"
						<?php echo 'title="'.$html['country_name'].' ('.$html['country_code'].'), '.$html['city_name'].' ('.$html['region_name'].')"';?>><img
							src="<?php echo get_absolute_url();?>202-img/flags/<?php echo strtolower($html['country_code']);?>.png"></span></td>
					<td class="isp"><?php if($html['isp_name']) echo $html['isp_name']; else echo "-"?></td>
					<td class="filter">
					<?php if ($click_row['click_filtered'] == '1') { ?>
						  <img style="margin-right: auto;"
						src="<?php echo get_absolute_url();?>202-img/icons/16x16/delete.png"
						alt="Filtered Out Click" title="filtered out click" /> 
					<?php } elseif ($click_row['click_lead'] != '0') { ?>
						  <img style="margin-right: auto;"
						src="<?php echo get_absolute_url();?>202-img/icons/16x16/money_dollar.png"
						alt="Converted Click" title="converted click" width="16px"
						height="16px" /> 
					<?php } else { ?>
						  <img style="margin-right: auto;"
						src="<?php echo get_absolute_url();?>202-img/icons/16x16/add.png"
						alt="Real Click" title="real click" /> 
					<?php } ?>
				</td>
					...
					<td class="referer_big"><div
							style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; width: 150px;"
							title="<?php if($html['referer']) echo $html['referer']; else echo "-";   ?>"><?php
					...
					<td class="keyword"><div
							style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; width: 250px;"
							title="<?php if($html['keyword']) echo $html['keyword']; else echo "-";   ?>"><?php if($html['keyword']) echo "<em>".$html['keyword']."</em>"; else echo "-"; ?></div></td>
	...
	<div class="col-xs-12 text-center">
		<div class="pagination" id="table-pages">
			...
			<?php if ($query['offset'] > 0) {
				    ...
				    $query['offset'] = 0; //set offset to 0 to start the counting;
				}
