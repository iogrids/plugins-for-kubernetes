...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($_POST['aff_campaign']) || empty($_POST['conversion_name']) || empty($_POST['time_from']) || empty($_POST['time_to'])) {
        ...
        $line=array('Parameters:TimeZone='.$timezoneOffset.';,,,,');
        
       // echo $line;
        $output = fopen("php://output", "w");     
        $header='### INSTRUCTIONS ###,,,,';
        fwrite($output, $header . "\r\n");
        $header='"# Important: Remember to change the TimeZone value (GMT offset) in the ""parameters"" row",,,,';
        fwrite($output, $header . "\r\n");
        $header='"# TimeZone values should be entered in HHMM format (e.g. New York is -0500 and Berlin is +0100. If you use Greenwich Mean Time, simply enter +0000).",,,,';
        fwrite($output, $header . "\r\n");
        $header='"# Make sure you do not include additional columns. For more instructions on how to use this tempate, visit https://aka.ms/dk8o1y",,,,';
        fwrite($output, $header . "\r\n");
        $header=',,,,';
        fwrite($output, $header . "\r\n");
        $header='### TEMPLATE ###,,,,';
		...
		//$line=str_replace('"', '', array('Parameters:TimeZone='.$timezoneOffset.';'));
	//	fputcsv($output, $line);
	
		$mysql['aff_campaign'] = $db->real_escape_string($_POST['aff_campaign']);
		$msclkid_sql = "SELECT aff_campaign_name, click_time, msclkid, click_payout, aff_campaign_currency FROM 202_clicks
					  JOIN 202_aff_campaigns USING (aff_campaign_id)
				      JOIN 202_bing USING (click_id)
		...
		$msclkid_results = $db->query($msclkid_sql);

		while ($msclkid_row = $msclkid_results->fetch_assoc()) {
			$campaign_name = $msclkid_row['aff_campaign_name'];
			fwrite($output, implode(',', array($msclkid_row['msclkid'], $_POST['conversion_name'], date('m/d/Y H:i:s', $msclkid_row['click_time']), number_format((float)$msclkid_row['click_payout'], 2, '.', ''), $msclkid_row['aff_campaign_currency'])). "\r\n");
		}

		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename=msclkid_'.$campaign_name.'_'.$_POST['time_from'].'_'.$_POST['time_to'].'.csv');
...
template_top('Export MSCLKID', NULL, NULL, NULL); ?>

<div class="row">
	<div class="col-xs-12">
		<h6>Export MSCKLID to .CSV</h6>
		<small>Sometimes, a click on a Bing ad doesn't lead directly to an online sale, but instead starts a customer down a path that ultimately leads to a sale in the offline world, such as at your office or over the phone. By importing offline conversions, you can measure what happens in the offline world after a click on one of your ads.</small>
