...
if($_SESSION['publisher']==false){ //user is able to see all campaigns
    $mysql['user_id_query'] = " != '0' ";
}
else{
    $mysql['user_id_query'] = " = '" . $_SESSION['user_own_id'] . "' "; //user can only see thier campaigns
}



$campaign_sql = "SELECT aff_campaign_id, aff_campaign_name FROM 202_aff_campaigns WHERE user_id".$mysql['user_id_query']." AND aff_campaign_deleted = '0'";
...
$logs_sql = "SELECT 2ca.aff_campaign_name, 2l.click_id, 2l.transaction_id, 2l.click_payout, 2l.click_time, 2l.conv_time, 2l.time_difference, 2l.ip, 2l.pixel_type, 2l.user_agent, 2l.deleted FROM 202_conversion_logs AS 2l LEFT JOIN 202_aff_campaigns AS 2ca ON (2l.campaign_id = 2ca.aff_campaign_id) WHERE 2l.user_id".$mysql['user_id_query']." AND 2l.click_time >= ".$mysql['from']." AND 2l.click_time < ".$mysql['to'];
