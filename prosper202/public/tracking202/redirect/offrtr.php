...
if( (null == (getCookie202('tracking202subid'))) || !is_numeric(getCookie202('tracking202subid'))){ //if no subid cookie found look for it in past 48 hours
	$mysql['ip_address'] = $ip_address->address;
	$daysago = time() - 86400 * 2; // past 48 hours
	$subid_sql="SELECT MAX(click_id) as click_id
	...
	LEFT JOIN 202_ips_v6 AS 2i6 ON (2i6.ip_id = 202_ips.ip_address COLLATE utf8mb4_general_ci)
...
if(!isset($mysql['click_id']) || !is_numeric($mysql['click_id'])) {
    die();
}
...
$mysql['ipqs_api_key'] = $db->real_escape_string($rotator_row['ipqs_api_key']);
...
switch ($rotator_row['user_keyword_searched_or_bidded']) { 
	...
	case "bidded":
		...
		} elseif (isset($_GET['OVKEY'])) { //if this is a Y! keyword
	...
	foreach ($criteria_row as $criteria) {
		...
		if (in_array('ALL', $values) || in_array('all', $values)) {
			...
			switch ($criteria['type']) {
				case 'fraud':
				$config = ['key'=>$mysql['ipqs_api_key'],'ip'=>$ip_address,'user_agent'=>$_SERVER['HTTP_USER_AGENT'],'language'=>$_SERVER['HTTP_ACCEPT_LANGUAGE']];
				$ipqs = new FraudDetectionIPQS($config);
				
				try {
					if($ipqs->isFraud($ip_address->address)){
						$fraud = 'true';
					}else{
						$fraud = 'false';
					}
...
if ($default == false) {
		...
		if (count($rule_redirects_row) > 1) {
			...
			if ($redirect_array['type'] == 'campaign') {
				...
				if($rule_redirect_row['aff_campaign_rotate']){
					$redirect_site_url = rotateTrackerUrl($db, $rule_redirect_row);
				}else{
					$redirect_site_url = $redirect_array['aff_campaign_url'];
				}
				...
				$redirect_site_url = replaceTrackerPlaceholders($db, $redirect_array['landing_page_url'], $mysql['click_id'],$mysql);	
				header('location: ' . $redirect_site_url);
				die();
		...
		if ($rotator_row['default_campaign'] != null) {
			...
			$redirect_site_url = replaceTrackerPlaceholders($db, $rotator_row['landing_page_url'], $mysql['click_id'],$mysql);	
			header('location: ' . $redirect_site_url);
			die();
