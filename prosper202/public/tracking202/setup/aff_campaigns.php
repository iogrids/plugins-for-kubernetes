...
if ($_SERVER ['REQUEST_METHOD'] == 'POST') {
	...
	if (empty ( $aff_campaign_url )) {
		$error ['aff_campaign_url'] = '<div class="error">What is your campaign url?</div>';
	}
	

	if ((strpos ( $_POST ['aff_campaign_url'], '://' ) === 'false')) {
		$error ['aff_campaign_url'] .= '<div class="error">Your Campaign URL must be a valid webpage or deeplink</div>';
	}
	...
	//check to see if they are the owners of this Category
	...
	if ($aff_network_result->num_rows == 0) {
		$error ['wrong_user'] = "<div class='error'>You are not authorized to add a campaign to another user's network</div>";
	} else {
...
template_top ( 'Campaign Setup', NULL, NULL, NULL );
?>
...
<div class="row" style="margin-bottom: 15px;">
	...
	<div class="col-xs-12">
		...
		<span style="display:none" id="help-text"><br/>
			<span class="infotext">
				<em>If you do not understand how subids work at your network, stop, and contact your campaign manager.<br/>
					Prosper202 supports the ability to mask your traffic; Referrer Shield will protect your campmaings 
					from being copied by your partners because they won't  be able to see your keywords. 
					Please note, if you are doing direct linking with Google Adwords,  Referrer Shield can kill your
					quality score or get your ads disabled. If you don't understand Referrer Shield leave it off for now and
...
<div class="row">
	<div class="col-xs-7">
		<small><strong>Add Your Campaign</strong></small><br/>
		<span class="infotext">Add each of campaign you plan to promote.</span>
		...
		<form method="post" class="form-horizontal" action="<?php if ($delete_success == true) { echo $_SERVER ['REDIRECT_URL']; } ?>" role="form" style="margin:15px 0px;">
			...
			<div class="form-group " style="margin-bottom: 0px;">
				<label for="link_assistant" class="col-xs-4 control-label" style="text-align: left;">Bot202 Link Assistant:</label>
				<div class="col-xs-6">
                    <input type="checkbox" checked data-toggle="switch" name="link_assistant" id="link_assist" data-on-color="success" />
				</div>
			</div>		
			...
			<div class="form-group <?php if($error['aff_campaign_url']) echo "has-error";?>" style="margin-bottom: 0px;">
				<label class="col-xs-4 control-label" for="aff_campaign_url" style="text-align: left;">Campaign URL <span class="fui-info" data-toggle="tooltip" title="This is where people will be sent when your Prosper202 tracking link is clicked."></span></label>
				<div class="col-xs-6">
			...
			<div class="form-group" style="margin-bottom: 10px;">
				<div class="col-xs-6 col-xs-offset-4" id="placeholders">
					...
					<input type="button" class="btn btn-xs btn-primary" value="[[t202pubid]]"/>
					<br/><br/>
					...
					<input type="button" class="btn btn-xs btn-primary" value="[[msclkid]]"/>
					<input type="button" class="btn btn-xs btn-primary" value="[[fbclid]]"/>
				    <br/><br/>
			...
			<div class="form-group" style="margin-bottom: 0px;">
				<label for="aff_campaign_cloaking" class="col-xs-4 control-label" style="text-align: left;">Referrer Shield:</label>
				<div class="col-xs-6">
	...
	<div class="col-xs-4 col-xs-offset-1">
		<div class="panel panel-default">
			...
			<?php if($_SESSION['user_pref_ad_settings']!='hide_all'){?>
		      <iframe id="adframe" src="<?php echo TRACKING202_ADS_URL; ?>/prosper202-campaign/?t202aid=<?php echo $_SESSION['user_cirrus_link']; ?>" scrolling="no" frameborder="0"> </iframe>
		    <?php }?>
			<div id="campaignList">
			<?php 
			...
			<input class="form-control input-sm search" style="margin-bottom: 10px; height: 30px;" placeholder="Filter">
				<ul class="list">        
					...
					if($dniNetworks){
						...
						foreach ($dniNetworks as $dninetwork) {
							if($dninetwork['networkId']!='skip')
								echo "<li><img src='".$dninetwork['favIconUrl']."' width=16>&nbsp;&nbsp;<strong>".$dninetwork['name']." (Dynamic Network Integration)</strong> - <a href=".get_absolute_url()."202-account/api-integrations.php?add_dni_network=".$dninetwork['networkId'].">Activate</a></li>";
						}}
					...
					while ( $aff_network_row = $aff_network_result->fetch_array (MYSQLI_ASSOC) ) {
						...
						//print out the individual accounts per each Traffic Source
...
$(document).ready(function() {
	...
	var linkAssist = function (url) {
		...
		['/paykstrt\.com(\/){1}[0-9]+\/{1}[0-9]+\/?(tid=)?', 'tid'],
	...
	$("#aff_campaign_url").on("input change", function() {
		if($('input[name="link_assistant"]').bootstrapSwitch('state')){ //only run the link assist if it's turned on
