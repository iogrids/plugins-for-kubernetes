...
function about_revenue_upload() { 
	...
	echo '<div class="row">
			<div class="col-xs-12">
				...
				echo '</h6>
				<small>This area allows you to upload the revenue reports. You can now upload the exact sale amount that each subid generated,  you can now upload the exact revenue that was generated per subid.  This is specifically helpful if you are receiving a commission based on a percentage of sales or if you constantly get more than one lead for each subid.</small>
			</div>
		</div>
	...
	case 2:
		...
		while ($row = @fgetcsv($handle, 100000, ",")) {
			...
			if (is_numeric($click_id)) { 
				...
				addConversionLog($mysql['click_id'], '', $mysql['aff_campaign_id'], $mysql['click_payout'], $mysql['user_id'], $mysql['click_time'], $ip_address, $_SERVER['HTTP_USER_AGENT'],'', 0);
